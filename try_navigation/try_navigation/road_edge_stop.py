...
パイプライン:
  /pcd_segment_ground → odom補正つき短期蓄積(Nフレーム)
    ...
    → 追跡（前回の基準線とゲート判定 → 1次フィルタで平滑化）
    → 停止目標距離を算出 → N回連続一致で停止フラグ

追跡について:
  基準線 L をグローバル座標で1つだけ保持する（点群は保持しない）。
  毎フレーム現在姿勢基準に変換して今回の検出と比較し、
  track_gate 以内なら平滑化して更新、外れたら棄却して前回値を維持する。
  縞の間隔(約0.7m)の半分をゲートにしてあるため、隣の縞への乗り換えを弾ける。

方位について:
  絶対的な向き（進行方向との角度）は問わない。
  「縞どうしが互いに平行であること」を条件にするため、真横からでも検出できる。
"""
...
class RoadEdgeStop(Node):
    ...
    def __init__(self):
        ...
        # 形状ゲート（向きは問わない）
        self.declare_parameter('min_long_length', 1.7)
        self.declare_parameter('min_short_width', 0.20)
        self.declare_parameter('max_short_width', 0.80)
        # 連続性ゲート
        self.declare_parameter('min_stripe_count', 3)
        self.declare_parameter('pitch_min', 0.50)
        self.declare_parameter('pitch_max', 1.50)
        self.declare_parameter('pitch_tolerance', 0.10)
        self.declare_parameter('max_parallel_dev_deg', 15.0)
        # 追跡・平滑化
        self.declare_parameter('track_gate', 0.35)        # 同一とみなす距離 [m]
        self.declare_parameter('track_angle_gate', 20.0)  # 同一とみなす角度 [deg]
        self.declare_parameter('smooth_alpha', 0.4)       # 小さいほど滑らか
        self.declare_parameter('track_reject_max', 3)     # 連続棄却で乗り換え
        # 停止幾何
        self.declare_parameter('offset_d', 0.30)
        self.declare_parameter('safety_margin', 0.50)
        self.declare_parameter('robot_front_x', 0.45)
        self.declare_parameter('near_edge_percentile', 15.0)
        self.declare_parameter('min_normal_x', 0.20)
        # 判定
        ...
        # 追跡状態（グローバル座標）
        self.trk_p = None        # 基準線上の点 [x, y]
        self.trk_n = None        # 法線ベクトル
        self.trk_reject = 0
    ...
    def detect(self, local, msg):
        ...
        head = chain[0]                       # 最も手前の縞
        meas_s, meas_v, meas_n = head['near'], head['v'], head['n']

        # --- 追跡・平滑化 ---
        near_s, v_hat, n_hat, trk_state = self.track(meas_s, meas_v, meas_n)
        if near_s is None:
            if gp('verbose').value:
                self.get_logger().info('frames=%d pts=%d clusters=%d pass=%d chain=%d %s'
                                       % (len(self.frame_buf), roi.shape[1], n_cluster,
                                          len(cands), len(chain), trk_state))
            self._no_detection()
            return
        ...
        if gp('verbose').value:
            ...
            self.get_logger().info(
                'frames=%d pts=%d cl=%d pass=%d chain=%d gaps=[%s] %s | L=%.2f edge=%.2f rem=%.2f hit=%d'
                % (len(self.frame_buf), roi.shape[1], n_cluster, len(cands),
                   len(chain), gs, trk_state, x_L, x_edge, remaining, self.hit_count))
    ...
    # ------------------------------------------------------------
    #  追跡（グローバル座標で基準線を1つだけ保持する）
    # ------------------------------------------------------------

    def track(self, meas_s, meas_v, meas_n):
        """戻り値: (平滑化後 near_s, v̂, n̂, 状態文字列)
        near_s が None のときは棄却（この回は出力しない）"""
        gp = self.get_parameter
        gate = gp('track_gate').value
        agate = gp('track_angle_gate').value
        alpha = gp('smooth_alpha').value
        rej_max = int(gp('track_reject_max').value)

        Rg = rot2d(self.yaw)
        pos = np.array([self.px, self.py])

        # 今回の観測（ローカル）→ グローバル
        meas_p_l = meas_n * meas_s          # 基準線上で原点に最も近い点
        meas_p_g = Rg @ meas_p_l + pos
        meas_n_g = Rg @ meas_n

        # 初回はそのまま採用
        if self.trk_p is None:
            self.trk_p, self.trk_n = meas_p_g, meas_n_g
            self.trk_reject = 0
            return meas_s, meas_v, meas_n, 'trk=init'

        # 前回値をローカルに戻して比較
        Rl = rot2d(-self.yaw)
        prev_n_l = unit(Rl @ self.trk_n)
        prev_s = float(prev_n_l @ (Rl @ (self.trk_p - pos)))

        d_dist = abs(meas_s - prev_s)
        d_ang = angle_between_axes(meas_n, prev_n_l)

        if d_dist > gate or d_ang > agate:
            self.trk_reject += 1
            if self.trk_reject >= rej_max:
                # 連続で外れ続けるなら乗り換える
                self.trk_p, self.trk_n = meas_p_g, meas_n_g
                self.trk_reject = 0
                return meas_s, meas_v, meas_n, 'trk=switch'
            # 前回値を維持して出力（観測は捨てる）
            if abs(prev_n_l[0]) < 1e-6:
                return None, None, None, 'trk=reject'
            prev_v_l = np.array([-prev_n_l[1], prev_n_l[0]])
            return prev_s, prev_v_l, prev_n_l, 'trk=hold%d' % self.trk_reject

        # ゲート内 → 平滑化して更新
        self.trk_reject = 0
        s_new = (1.0 - alpha) * prev_s + alpha * meas_s
        n_new = unit((1.0 - alpha) * prev_n_l + alpha * meas_n)
        if n_new[0] < 0:
            n_new = -n_new
        v_new = np.array([-n_new[1], n_new[0]])

        self.trk_p = Rg @ (n_new * s_new) + pos
        self.trk_n = Rg @ n_new
        return s_new, v_new, n_new, 'trk=ok'
