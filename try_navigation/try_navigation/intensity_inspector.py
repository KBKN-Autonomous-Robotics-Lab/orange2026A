...
class IntensityInspector(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'subscribing: {self.topic}  accum={self.accum}')
    ...
    def get_accumulated(self):
        """直近 accum フレームを連結。センサ座標のまま（静止前提）"""
        with self._lock:
            if not self._buf:
                return None, 0
            frames = list(self._buf)[-self.accum:]
        return np.concatenate(frames, axis=0), len(frames)
...
class Viewer:
    ...
    HELP = '[ ]=accum  a/d=profile x  r=autoscale  f=0-255  s=save  q=quit'
